...
@cloud_tasks_bp.route("/start_crawl_backwards/e86fc4d3b30d4f4f947f13c0a128377d")
def start_crawl_backwards():
    """This function is called by the app engine cron job to start the crawl loop. See cron.yaml
    All this function does is create the initial gcloud http task with the first zipcode
    @return:
    """
    try:
        logging.info("[+] Fetching zip_codes.txt from Google Cloud Storage")
        last_zip_code = GcloudStorageService.get_last_zipcode()
        logging.info(f"[+] Creating gcloud http task with first zipcode: {last_zip_code}")
        GcloudTasksService.create_zipcode_task(last_zip_code, 1, descending=True)
        return "success"
    except Exception:
        logging.error(traceback.format_exc())
        return "error", 500
...
@cloud_tasks_bp.route("/continue_crawl_backwards/dfc2d27b2dbb4417926d8e396737a76a", methods=["POST"])
def continue_crawl_backwards():
    ...
    try:
        ...
        next_zipcode = GcloudStorageService.get_previous_zipcode(current_zipcode)
        if next_zipcode:
            GcloudTasksService.create_zipcode_task(next_zipcode, 1, descending=True)
        return "success"
    except Exception:
        logging.error(traceback.format_exc())
        return "success"
